...
Objetivo: Automatizacao dediagnostico ambiental de imoveis rurais do projeto P4F. 
Versao: 1.0
Data: 27/11/2022
'''
...
def create_fgdb(v_output, v_sptial_ref):
    ...
    return dataset_input, dataset_intermediate_results, dataset_intermediate_results_outputs, dataset_outputs, dataset_outputs_calculate


def add_input(inp_lakes, inp_water_bodies, inp_rivers, inp_properties, inp_springs, inp_veg_2008, inp_veg_current, inp_ppa_areas, inp_saf, inp_others_uses, output):
    ...
    return nm_lakes, nm_water_bodies, nm_rivers, nm_properties, nm_springs, nm_veg_2008, nm_veg_current, nm_ppa_areas, nm_saf, nm_others_uses
    

def config(v_lakes, v_water_bodies, v_rivers, v_properties, v_output):  # 1. Modulo config
...
def app_consolidada(v_properties, v_springs, v_output): # 2. Modulo app_consolidada
...
def app_total(v_water_bodies, v_rivers, v_springs, v_input, v_output):
    ...
    if input_ppa_areas == "":       
        ...
        management.Merge(inputs=[app_total_wb, app_total_springs, app_total_lagoa_maior_20, app_total_lagoa_1_20, app_total_rivers_2, v_ppa_areas], output=app_total_Merge) # "v_ppa_areas" constava no merge, mas não existe essa camada no GDB
...
def properties(v_lakes, v_veg_current, v_saf, v_others_uses, v_properties_join, v_output, v_sptial_ref):
...
module_input = add_input(input_lakes, input_water_bodies, input_rivers, input_properties, input_springs, input_veg_2008, input_veg_current, input_ppa_areas, input_saf, input_others_uses, dataset_list[0])
...
module_properties = properties(module_input[0], module_input[6], module_input[8], module_input[9], module_calculate, dataset_list[3], sptial_ref)
